...
- Take the complexities and complications within the natural world and its data and make it simpler or easier to manipulate/understand
- what do we want to know?: is a course not working? is a decrease in enrollment numbers a measure of form problems in the course?
-How do we measure this?: difference in enrollment numbers from september 1 to today
- we need to use just the 2 data frames
- Narrow down to: course id and undergrad enrollment numbers, only other variable we have is between the two data frames which is time


After we have a model, Temperance suggests we should:
- Temperance is the practice of self-control, moderation, and useful in recognizes the limitation of our models
- Our model is only shows decrease in enrollment but not exactly the root cause of it. We can infer whether it is a form problem with the class or just a natural student body problem or something else entirely. Our data only explains the trend of enrollment not why its happening. 
...
```{r}
sep_difference <- full_join(sep_old, sep_new, by = c("id", "name", "title", "department")) %>%
  mutate(u_grad.diff = (u_grad.y - u_grad.x)) %>%
  arrange(u_grad.diff)
sep_difference
```
